...
set NSudo="%Konum%\Bin\NSudo.exe" -U:T -P:E -Wait -ShowWindowMode:hide cmd /c
	if %errorlevel%==1 ("%Konum%\Bin\NSudo.exe" -U:E -ShowWindowMode:hide cmd /c Powershell -command "Start-Process '%Konum%\readyTool.bat'"
						exit)
...
echo %R%[90m┌──────────────────────────────────────────────────────────┐%R%[0m
echo %R%[90m│%R%[31m  1)%R%[0m Uygulama İndir                                       %R%[90m│%R%[0m
echo %R%[90m│%R%[31m  2)%R%[0m Güncellemeleri Ertele                                %R%[90m│%R%[0m
echo %R%[90m│%R%[31m  3)%R%[0m Discord Sunucum                                      %R%[90m│%R%[0m
echo %R%[90m│%R%[31m  4)%R%[0m Windows Etkinleştirme (Home)                         %R%[90m│%R%[0m
echo %R%[90m├──────────────────────────────────────────────────────────┤%R%[0m
echo %R%[90m│%R%[31m  X)%R%[0m Çıkış                                                %R%[90m│%R%[0m
echo %R%[90m└──────────────────────────────────────────────────────────┘%R%[0m
set /p main= %R%[36m ► İşlem Numarası %R%[0m
	if %main%==1 (goto AppList)
	if %main%==2 (goto Update2099)
	if %main%==3 (start https://dsc.gg/readyos)
	if %main%==4 (goto Activation)
	if %main%==x (exit)
	if %main%==X (exit)
goto MainMenu
...
:AppList
cls
mode con cols=60 lines=54
title Ready App Installer
echo %R%[36m
echo  ██████╗ ███████╗ █████╗ ██████╗ ██╗   ██╗ ██████╗ ███████╗
echo  ██╔══██╗██╔════╝██╔══██╗██╔══██╗╚██╗ ██╔╝██╔═══██╗██╔════╝
echo  ██████╔╝█████╗  ███████║██║  ██║ ╚████╔╝ ██║   ██║███████╗
...
echo %R%[90m│%R%[31m 27)%R%[0m IOBit Driver Booster                                 %R%[90m│%R%[0m
echo %R%[90m│%R%[31m • İletişim%R%[0m                                               %R%[90m│%R%[0m
echo %R%[90m│%R%[31m 28)%R%[0m Discord                                              %R%[90m│%R%[0m
echo %R%[90m│%R%[31m 29)%R%[0m WhatsApp                                             %R%[90m│%R%[0m
echo %R%[90m│%R%[31m 30)%R%[0m Zoom                                                 %R%[90m│%R%[0m
echo %R%[90m│%R%[31m • Kodlama%R%[0m                                                %R%[90m│%R%[0m
echo %R%[90m│%R%[31m 31)%R%[0m Notepad++                                            %R%[90m│%R%[0m
echo %R%[90m│%R%[31m 32)%R%[0m Python                                               %R%[90m│%R%[0m
echo %R%[90m├──────────────────────────────────────────────────────────┤%R%[0m
...
set /p applist= %R%[36m ► Uygulama Numarası: %R%[0m
	...
	if %applist%==27 (set AppID=driverbooster&set Title=IOBit Driver Booster&set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==28 (set AppID=discord&set Title=Discord&set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==29 (set AppID=WhatsApp&set Title=WhatsApp&set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==30 (set AppID=zoom&set Title=Zoom&set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==31 (set AppID=notepadplusplus&set Title=Notepad++ &set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==32 (set AppID=python&set Title=Python&set AppMenu_X_Link=AppList&goto AppMenu)
	if %applist%==X (goto MainMenu)
	if %applist%==x (goto MainMenu)
goto AppList
...
echo %R%[90m├──────────────────────────────────────────────────────────┤%R%[0m
echo %R%[90m│%R%[31m  X)%R%[0m Geri Dön                                             %R%[90m│%R%[0m
echo %R%[90m└──────────────────────────────────────────────────────────┘%R%[0m
...
:Update2099
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v "PauseUpdatesExpiryTime" /t REG_SZ /d "2099-11-11T16:38:59Z" /f > NUL 2>&1
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v "PauseFeatureUpdatesEndTime" /t REG_SZ /d "2099-11-11T11:11:11Z" /f > NUL 2>&1
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v "PauseQualityUpdatesEndTime" /t REG_SZ /d "2099-11-11T11:11:11Z" /f > NUL 2>&1
goto MainMenu

:Activation
cls
echo Bu adımda internete bağlı olmanız gerekmektedir. Devam etmek için herhangi bir tuşa basın...
pause > NUL
