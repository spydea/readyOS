...
cls
title readyOS Başlangıç Optimizasyonu
for /F "tokens=1,2 delims=#" %%a in ('"prompt #$H#$E# & echo on & for %%b in (1) do rem"') do (set R=%%b)
cd /d "%~dp0"
for /f %%a in ('"cd"') do set Konum=%%a
reg query "HKU\S-1-5-19" > NUL 2>&1
	if %errorlevel% EQU 1 (Call :Powershell "Start-Process '%Konum%\readyStart.bat' -Verb Runas"&exit)
...
echo  ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═════╝    ╚═╝    ╚═════╝ ╚══════╝
echo.
echo %R%[33m İşaretçi paketi güncelleniyor...%R%[0m
RunDll32 advpack.dll,LaunchINFSection C:\readyOS\CursorPack\Install.inf,DefaultInstall
cls
mode con cols=60 lines=10
echo %R%[36m
echo  ██████╗ ███████╗ █████╗ ██████╗ ██╗   ██╗ ██████╗ ███████╗
...
echo  ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═════╝    ╚═╝    ╚═════╝ ╚══════╝
echo.
echo %R%[33m Masaüstü dosyaları çıkartılıyor...%R%[0m
...
bcdedit /set {current} description readyOS 22H2 >nul 2>&1
...
RD /S /Q "%Konum%\CursorPack" > NUL 2>&1
RD /S /Q %temp%\* > NUL 2>&1
DEL /F /Q /A %temp%\* > NUL 2>&1
echo.
echo %R%[92m İŞLEMLER TAMAMLANDI %R%[0m
shutdown /r
DEL /F /Q /A "%Konum%\readyStart.bat" > NUL 2>&1
exit
